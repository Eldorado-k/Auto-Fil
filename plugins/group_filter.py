...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⬅️ Retour", callback_data=f"next_{req}_{key}_{off_set}"),
        ...
        btn.append(
            [InlineKeyboardButton(f"❄️ {math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)}", callback_data="pages"),
             InlineKeyboardButton("Suivant  ➡️", callback_data=f"next_{req}_{key}_{n_offset}")])
    else:
        btn.append(
            [
                InlineKeyboardButton("⬅️ Retour", callback_data=f"next_{req}_{key}_{off_set}"),
...
async def advantage_spoll_choker(bot, query):
    ...
    if k == False:
        ...
        if files:
            ...
            k = await query.message.edit("je n'ai rien trouvé dans ma base de données pour ta demande. fait une demande au près de l'admin")
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"❄️ Pages 1/{math.ceil(int(total_results) / 6)}", callback_data="pages"),
             InlineKeyboardButton(text="➡️ Suivant ", callback_data=f"next_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
    ...
    if imdb:
        ...
        cap = f"je n'ai rien trouvé pour ta demande {search}"
...
async def advantage_spell_chok(msg):
    ...
    if not movielist:
        k = await msg.reply("je n'ai rien trouvé pour ta demande. corrige les mots ou le mot")
        await asyncio.sleep(8)
        return await k.delete()
    ...
    await msg.reply("je n'ai rien trouvé pour ta demande. clique sur l'un des boutons d'en bas", reply_markup=InlineKeyboardMarkup(btn))
