...
async def pm_next_page(bot, query):
    ...
    if not search: return await query.answer("Tu utilises un de mes anciens messages. Refait une autre demande ", show_alert=True)
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⬅️ Retour", callback_data=f"pmnext_{req}_{key}_{off_set}"),
...
async def pm_spoll_tester(bot, query):
    ...
    if not movies:
        return await query.answer("Tu utilises un de mes anciens messages. Refait une autre demande ", show_alert=True)
...
async def pm_AutoFilter(client, msg, pmspoll=False):    
    ...
    if imdb:
        ...
        cap = f"Voici ce que j'ai trouvé pour ta demande {search}"
...
async def pm_spoll_choker(msg):
    ...
    gs_parsed = []
    if not g_s:
        k = await msg.reply("Je n'ai rein trouvé pour ce nom", quote=True)
        await asyncio.sleep(10)
    ...
    if not movielist:
        k = await msg.reply("Je n'ai rien trouvé de relative avec ce nom", quote=True)
        await asyncio.sleep(10)
        return await k.delete()
    temp.PM_SPELL[str(msg.id)] = movielist
    btn = [[InlineKeyboardButton(text=movie.strip(), callback_data=f"pmspolling#{user}#{k}")] for k, movie in enumerate(movielist)]
    btn.append([InlineKeyboardButton(text="Close", callback_data=f'pmspolling#{user}#close_spellcheck')])
    await msg.reply("Je n'ai rein trouvé de relative pour ça. ? Clique sur l'un des boutons ci dessous", reply_markup=InlineKeyboardMarkup(btn), quote=True)
