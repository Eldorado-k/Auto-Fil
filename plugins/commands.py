...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[           
            InlineKeyboardButton('📢 Supprot 📢', url=f'https://t.me/{SUPPORT_CHAT}')
            ],[
    ...
    if len(message.command) != 2:
        buttons = [[
            InlineKeyboardButton("Anime Terr, url=f"https://t.me/Anime_Terr")
            ],[
        ...
        m = await message.reply_sticker("CAACAgQAAxkBAAI-_2hySHHesLkUZoU9WqVnKOfx5wpHAAL_FgACUhHRU_6lv8TmSwi5HgQ") 
        await asyncio.sleep(2)
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        try:
            ...
            return
        btn = [[InlineKeyboardButton("Rejoins ma  Chaîne ✨", url=invite_link.invite_link)]]
        if message.command[1] != "subscribe":
            try:
                kk, file_id = message.command[1].split("_", 1)
                pre = 'checksubp' if kk == 'filep' else 'checksub' 
                btn.append([InlineKeyboardButton("⟳ Reesaie", callback_data=f"{pre}#{file_id}")])
            except (IndexError, ValueError):
                btn.append([InlineKeyboardButton("⟳ Réessaie ", url=f"https://t.me/{temp.U_NAME}?start={message.command[1]}")])
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help"]:
        buttons = [[
            ...
            InlineKeyboardButton("Recherche 🔎", switch_inline_query_current_chat=''), 
            InlineKeyboardButton(" Chaîne 🔈", url="https://t.me/BotZFlix")
            ],[      
